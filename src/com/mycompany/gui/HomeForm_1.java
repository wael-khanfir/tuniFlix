...
public class HomeForm_1 extends BaseForm {
    ...
    public HomeForm_1(Form current) {
...
int nbre=2;
  ...
  double[] values = new double[2];
        ...
        current=this;
        setTitle("Interface Produits");
         setLayout(BoxLayout.y());
        add(new Label("choose an option"));
        Button btnListProduit=new Button("List des produits");
        Button btnImprimer = new Button("Imprimer");
        
        //PDF
        btnImprimer.addActionListener( e -> {Form hi = new Form("PDF Viewer", BoxLayout.y());


    FileSystemStorage fs = FileSystemStorage.getInstance();
    String fileName = fs.getAppHomePath() + "Listeproduit.pdf";
    if(!fs.exists(fileName)) {
        Util.downloadUrlToFile("http://127.0.0.1:8000/pdf/wiem", fileName, true);
    }
    Display.getInstance().execute(fileName);
 });
         
        btnListProduit.addActionListener(e->new ListProduit(current).show());
       
        addAll(btnListProduit,btnImprimer);
